...
| [parity-state-producer.md](parity-state-producer.md) | **ST-02/ST-03/ST-04/ST-05** — the volatile `state` pillar: canonical encoder + domain-separated **Merkle roots** (`state_codec.py`+`state_merkle.py`, R3 subsystem tree `schemas/state-volatile-v1.json`) as the mechanism of a `view.json` producer + `adapt_state`, wired into `parity_prove`. **ST-04** first-divergence report (`state_diff.py`: leaf path + typed values + raw bits + last-matching frame + value transition + co-divergent leaves). **ST-05** semantic-mutation CONSUMER (`state_mutation.py` + `schemas/state-mutation-v1.json` + `reference/state-mutations.md`): reconstruct subtree / first-wrong-write / ordering invariant, fills ST-04's provenance seam. R3: `rngcalls` benign-excluded. | 2026-07-17 |
| [gx00-d3d-method-census.md](gx00-d3d-method-census.md) | **GX-00** — D3D8 method census (capture completeness): the v3 proxy's 113 vtable methods classified recorded/wrapper/query-only/forwarded-irrelevant/**render_affecting_unsupported** (33 forwarded, render-affecting, UNCAPTURED — the fail-closed RISK set: Reset, SetViewport, state-blocks, ProcessVertices, shaders, palettes, resource-creation). Lead: `SetPixelShader` forwarded while `SetVertexShader` recorded. `d3d_census.py` + drift guard `test_d3d_census.py`. STATIC census; the dynamic (which are actually called) is next. | 2026-07-17 |
| [gx03-resource-versions.md](gx03-resource-versions.md) | **GX-03/GX-04** — per-draw VB/IB resource versions (resolves the GX-00 arrprobe VIOLATION). Probe: arrprobe reuses 3 VB+3 IB/frame, 0 snapfail. Key: D3D8 CreateVertexBuffer has NO init-data ⇒ content set ONLY via Lock/Unlock ⇒ wrapping the buffers to intercept every Lock/Unlock is PROVABLY complete. Mechanism: wrap VB/IB (`WrapVB/WrapIB`), shadow at Unlock, FREEZE-at-bind into a per-frame arena. VALIDATED: arrprobe 80/80 bit-exact + census VIOLATION → SAFE; synthetic A,B,A fixture → 2 RES_VB (split+dedup). | 2026-07-17 |
| [gx06-graphics-corpus.md](gx06-graphics-corpus.md) | **GX-06** — graphics-capture regression corpus (the GX-arc capstone). Proves the record→REPLAY path for every recorded container OPCODE is correct + regression-guarded, two axes: a FIXTURE (synthetic controlled capture → bit-exact replay) + a REAL PROOF (real cached scenario → v3verify bit-exact). Sweep: DrawPrimitive/DrawIndexedPrimitiveUP/CopyRects UNOBSERVED (0/134 containers) ⇒ fixture-only. 2 new fixtures (gx06_sink=22 non-RT ops; gx06_rt=RT+4 SURFREF) + gx04/05 + 3 real proofs (title 120/120, arrprobe 1500/1500, pause 240/240). Gate `gx_corpus.py` FAST + `--verify`: COMPLETE. Closed the GX-05 diagnostic-reader residual. | 2026-07-17 |
| [parity-EP07-human-review.md](parity-EP07-human-review.md) | **EP-07** — bridge human confirmations: `human_review` → `canonical.NON_HASHED` (R3 canonicalization refinement, frozen schema shape unchanged) so a review is ADDITIVE + non-hashed. `prove.py:attach_human_review` (a confirming review over a non-PASS gate → `confirmed-despite-<MACHINE>`, never a silent pass) + `tools/parity_review.py` CLI (exit = the MACHINE gate). VERIFIED e2e on `house-firstcust-arrprobe` → `confirmed-despite-FAIL`. | 2026-07-17 |
