...
| [parity-state-producer.md](parity-state-producer.md) | **ST-02/ST-03** — the volatile `state` pillar: a canonical field encoder + domain-separated **Merkle roots** (`state_codec.py`+`state_merkle.py`, R3 subsystem tree `schemas/state-volatile-v1.json`) as the mechanism of a `view.json` producer (`state_producer.py`) + `adapt_state`, wired into `parity_prove`. R3: `rngcalls` benign-excluded (capture-origin, not a game global). Validated: arrprobe raw `rng` 1498/1498 + MORE sensitive than pixels (flags known companion residuals); `house-pause-save-commit` 198/198 Merkle-identical (contract-scope NOT_CAPTURED on the retail `--state` 2-frame head warm-up). | 2026-07-17 |
