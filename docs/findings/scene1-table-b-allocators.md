...
|------|-------|-------------------|----------:|-------|
| C8j.5 | **skeleton + common preamble + 3 anchor types (both allocators)** | 0044376a: 0x24, 0x66, 0x60 (preamble-only / minimal) + 00445a8c: 0xe, 0x97, 0x46 (LAB_00447584 tail-share); + scene1_record_b_spawn_entity() + scene1_record_b_spawn_npc() public API | ~400 | Wires both allocators behind feature flags.  Trace ring buffer (mirroring scene1_spawn.c) for instrumentation.  No type bodies — just preamble + LAB_004457e7-equivalent tail. |
| C8j.6 | **sin/cos drift cluster + cluster A (0044376a only)** | 0044376a: 2, 3, 4, 0x22, 0x54, 0x67 (drift) + 0x4d-0x50, 0xa5-0xa6, 99, 0x51, 0x52, 0x53 (cluster A) | ~450 | Largest single-allocator chip.  init_type_drift_cluster_a() + init_type_cluster_a(per-type +0xb4/local_10 table).  Argless cos pairs at L41148, L41296, L41379 verified per #7.  17 types covered. |
| C8j.7 | **cluster B + mega-cluster A (0044376a) + cluster B (00445a8c)** | 0044376a: 0x73/0x76/0x77/0x78/0x7a/0x7b/0x7c/0x7e (mega A) + 00445a8c: 0x4d/0x4e/0x4f/0x50/0xa5/0xa6 (cluster B) | ~400 | 14 types covered.  Mega-cluster A's "angle table mod 32" pattern is the biggest shared body in 0044376a.  Stand-in for owner+0xe3c (frame-offset) needed; likely just frame_counter alias. |
| C8j.8 | **NPC-table-deref + camera-yaw + matrix-init (0044376a)** | 0044376a: 0x23 (matrix), 0x29 (people-table), 0x30 (reverse-yaw + people target), 0x9b, 0x9d (NPC-bend), 0x82, 0x3e, 0x5f, 0x60 (group tail) | ~400 | Resolves DAT_0076bd54 stand-in for table B side.  9 types covered.  Adds the thunk_FUN_004a35d3 matrix port (small, ~80 lines) or stand-in. |
