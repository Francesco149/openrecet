...
| C8j.10 | **00445a8c first batch — NPC single-spawn (matrix-init + drift + cap-1 + player-aim)** — LANDED 2026-05-24 | 00445a8c: 0x56, 0x53, 0x51, 0x68 | ~250 | 4 types covered.  0x56's argless `thunk_FUN_004a2a03` (D3DXMatrixMultiply) verified via raw asm at 0x445c86 — 3 stack pushes (out=MATRIX0, a=local_8c, b=MATRIX0); composed slot.MATRIX0 = RotY(rot_y) × RotX(rot_x).  0x53 skips AUX_C8=1 (LAB_0044757e bypasses LAB_004469d2 — preamble 0 carries).  0x51 keeps the cluster-B 4-shift table even though cap=1 makes shifts unreachable (faithful port).  0x68 NPC variant aims at `g_scene1_player_pos` (DAT_056da1d8/dc/e0) — NO people-table scan (unlike entity 0x68).  5 host tests (1371 → 1376).  Deferred to C8j.11: mega-cluster B (~14 types incl. 0x96 / 0x84 player-aim w/ atan2). |
| C8j.10b | **00445a8c trivial-tail group B + explicit-return group** — LANDED 2026-05-24 | 00445a8c: 0x24, 0xa, 0xb, 0x14, 0x13, 0x99 (trivial-tail) + 0x1e, 0x88, 0x89, 0x9a, 0x9e (explicit-return) | ~80 | 11 types covered.  Trivial-tail group B mirrors the existing 0xe/0x97/0x46 anchor body (preamble-only).  Explicit-return group reads NEW owner field `+0x420` ("NPC current orientation" angle, used by ~10 other types in C8j.11+).  Engine raw decomp at L854-875 shows `sin+sin` / `cos+cos` for VEL (= sin*2 / cos*2 — same value but worth noting for diff-readers).  Engine's literal `goto LAB_00447584` is the loop tail; effectively a 1-particle return matching our cap=1 outer-loop semantics.  0x9e additionally writes LIFE_MULT=1.8 + SCALE_X=10.  2 host tests (1376 → 1378). |
| C8j.11 | **00445a8c mega-cluster B + player-aim w/ atan2** | 00445a8c: 0xa0, 0xa1, 0xa2, 0xa3, 0xa4, 0x73, 0x7a, 0x7c, 0x7e, 0x96, 0x84, 0xd/0x11/0x15/0xc/0x10, 0x16/0x17, 0x9c | ~500 | Mega cluster B shared body L42935+: NPC-bend pos sin/cos*1.2 + +1.3y; alt-pos sin/cos*0.8; 3-way owner+0x18 dispatch; per-type SCALE_X/LIFE_MULT.  Player-aim 0x84/0x96 needs `FUN_00503dd0` (atan2) wrapper.  Several types fall through into shared body via negative-test cascade. |
