...
**Status (2026-05-23):** **C8i ladder fully landed (C8i.0-5c).**  All
~134 per-type spawn handlers in FUN_00447f4f are now ported.  This
...
| 12    | the line-1240 mega-group (34 types — see below)      |
| 14    | 0x29 (`local_8 == 0xd`)                              |
...
## Line-1240 mega-group (34 types share one init body)

The dispatch at L1240 (after the parenthesis explosion) covers:
`0x25-0x28, 0x37-0x3a, 0x46-0x49, 0x7a-0x84, 0x86-0x90` — 4 + 4 + 4
+ 11 + 11 = **34 types** (the survey originally said "~60" — the
correct count from re-reading the if-chain is 34).  Note **0x85 is
NOT a member** of the predicate (it sits between the 0x7a-0x84 and
0x86-0x90 blocks but is excluded).

All share one randomized init body (small ground-skew velocity,
world-radial xz pos offset via sin/cos*amp, ground-skew lift on
pos.y, alternating-sign rot.y wobble per particle index, color =
u%10 in PARAM2, age=0, return after 12 particles).  Implemented in
C8i.4 as `init_type_mega_group()` + the `is_mega_group_type()`
range predicate.
...
| C8i.5a | 23 trivial-tail + sim-deref + 1-particle const types | ~200 landed | LAB_0044ad44/ad57/ad6a/ad72/ad77 epilogue group + 6/7/8/9 (DAT_056dae84 stand-in) + 0x11/0x12/0x54/0x50 (1-particle const-vel).  Resolves pending-human-check items #3 and #6 via raw-asm read of integrator chain-spawn call sites.  39 new tests (1082 → 1121 total). |
| C8i.5b | LAB_0044a43d shared body (11 types) + 0x67 + 0x59 + 0x71 + 0xf | ~250 landed | `init_type_shared_unit_half(int vy_bias)` factors the engine's L1370-1418 body: vel.{x,z}=trig(angle)*u*0.5 + vel.y=(u+bias)*0.5 + rot.z=u*2π.  Covers 5/0x5c/0x6f/10/0xb/0xc (main entry) + 0xe/0x2b/0x1b/0x3b/0x76 (jump to LAB_0044a43d) at bias=-0.5, and 0x67 inlined at bias=+0.1.  0x59 is a `param_7`-count anchor-back radial: mag=(u+2)*0.3, vy=(u+0.5)*0.2, pos.{x,z}-=vel*20, pos.y+=0.5 (no scale factor anywhere).  0x71 is a `param_7`-count centered radial: vel=(sin*2(u+0.2)*SCALE*0.5, u*SCALE*1.25, cos*2(u+0.2)*SCALE*0.5), PARAM1=u%100+0x14 life cap, AGE=-i stagger.  0xf is the only non-`param_7` member — 8 particles via LAB_0044acd2, (u±0.5)*3.2 cube vel, (u+0.1)*0.8 vy, AGE=-i, plus one dead `rng_next15` call kept for PRNG-sequence parity.  25 new tests (1121 → 1146 total). |
| C8i.5c | complex multi-stage with camera-yaw trig (9 types) | ~430 landed | 0x15/0x16/0x18/0x58/0x4f/0x3f/0x56/0x10/0x91 — all 9 param_7-count.  `init_type_15` (cube vel + AGE), `init_type_16` (cube vel + world-radial xz + pos.y lift via 2*(u-0.5) using ONE u twice), `init_type_18` (vel.x bias + inner-angle pos jitter w/ u BETWEEN sin/cos + sin/cos(-yaw)*5 bend + pos -= vel*60), `init_type_58` (wider amp than 0x18 + pos -= vel*200; PARAM2=400 w/ 50/50 fold to pos+=400*vel + vel.y flip + PARAM2=200; NO AGE write), `init_type_4f` (sin/cos(π/2) constant-angle vel → vel.x=mag, vel.z=0; inner-angle pos jitter; PARAM2=100), `init_type_3f_56` (shared body: vel.x/z=0 + positive vy + cube xz jitter + sin/cos(-yaw)*15 bend), `init_type_10_91` (cube vel w/ vy bias *0.98; PARAM1 = (rng15()&0xf)-2 signed [-2..13]; `param_7 < 10` → vel.y *= 0.7 + aux_15=0, else aux_15=1 — only C8i.5c handler that writes aux_15).  All camera-yaw reads use existing `g_scene1_camera_yaw` (engine `_DAT_073de39c`).  Argless cos sites resolved by pending-check #7's general proof.  26 new host tests (1146 → 1172 total).  Final chip; closes the C8i ladder. |

After C8i.5c lands, all ~134 per-type spawn handlers are covered and
