...
| C8i.5c | complex multi-stage with camera-yaw trig (~8 types) | ~150 | 0x15/0x16/0x18/0x58/0x4f/0x3f/0x56/0x10/0x91 — sin/cos pos jitter + camera trig (reads `g_scene1_camera_yaw` = engine `_DAT_073de39c`) + PARAM2=400/100 + 0x10/0x91 slot_hint pointer branches on `aux_15` flag.  Final chip; closes the C8i ladder. |
