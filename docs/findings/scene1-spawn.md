...
| C8i.3b | mixed-shape multi-particle radials (lines 289-527): 0x53, 0x4a, 0x43, 0x97, 0x96, 0x40, 0x36, 0x74, 0x4e | ~200 | 0x97/0x96 are 64-spawn spherical with secondary u*1.57 elevation; 0x96 adds camera-angle bend via DAT at param_1+0x948. 0x4a writes matrix rot seeds + PARAM1=param_7 + AGE=local_8*-4 (8-spawn via LAB_0044acd2). 0x36/0x74 are param_7-count. |
| C8i.3c | local_8-azimuth + chain pair (lines 528-735): 0x34, 0x35, 0x2c, 0x29, 0x32, 0x4c, 0x55, 0x4b, 0x33, 0x4d, 0x51, 0x57, 0x3e | ~200 | 0x34 chain-spawns 0x35 via scene1_spawn from particles_tick (already test-covered there). 0x32/0x4c/0x55/0x4b/0x33/0x4d/0x51/0x57/0x3e all encode azimuth as `local_8 * π + offset` (different offsets and counts). |
