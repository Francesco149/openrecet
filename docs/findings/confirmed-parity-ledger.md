...
| **Tear (companion) appearance** | "slightly off" — a **persistent** known issue. **NARROWED 2026-06-05 (user-confirmed):** with the bg-NPCs and free-roam RNG now 1:1, Tear is the **lone remaining free-roam sprite delta** and the user confirms it is **TWO things: the sprite-animation phase AND the wing particles** ("you can see both the sprite animation phase differs and the wing particles"). Drift bbox x[522-567] y[370-441] at `house-idle-npc-drift` cap_00, ~2000px@mean0.95, **present even +2 frames post-`{phasepin}`** (so it is NOT just the load-phase origin the pin zeroes — a genuine spring-follow-position and/or wing-flap+particle phase divergence). | **TOP next free-roam target** (user, 2026-06-05: "we should probably look into tear phase"). Do NOT assert it's purely "position" — it's anim-phase + particles. Needs a dedicated companion-controller investigation like the bg-NPC one. ([[project_next_char_controller]]) |
