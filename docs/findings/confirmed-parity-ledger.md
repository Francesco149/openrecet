...
|---|---|---|
| **Dialogue box-edge halo / 2D-UI ±1 LSB** | a ~1px halo around the `ive_window.tga` bubble border vs retail. Everything inside/outside the box is pixel-exact; only the frame edge differs (`intro-dialogue-lines` line-1 diff = box edge only). **Same class surfaced on the skip-prompt `savewindow.tga` banner (2026-06-05):** a uniform ~−1 LSB on the gold (port `[237,200,52]` vs retail `[238,201,52]`) + text edges. Geometry/UVs proven **bit-exact** vs `FUN_00404efc` (same truncation + half-texel insets), capture is bit-exact (static ceiling = 0 diff), so it's NOT geometry/filter/capture — it's a 2D-UI texture/colour-precision delta (TGA decode/upload or the MODULATE2X rounding). Invisible (±1). | user-flagged 2026-06-02 ("halo … likely scaling filter") + 2026-06-05 ("faint diff of the box, amplified by scaling"). Suspect POINT-vs-LINEAR / box-mip / TGA-decode ±1 (cf engine-quirks §54). Real delta — do NOT hand-wave. Chase: dump+diff the decoded `savewindow.tga`/`ive_window.tga` texels port-vs-retail. opening-prologue.md box-edge note. |
