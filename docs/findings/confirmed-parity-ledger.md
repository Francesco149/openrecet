...
|---|---|---|
| **Dialogue box-edge "halo" = box SCALE / bounce-anim PHASE divergence (ROOT-CAUSED 2026-06-05, was MIS-diagnosed)** | The `ive_window.tga` bubble "edge halo" is **NOT** a ±1-LSB texture/colour/decode/MODULATE delta and **NOT** sampler noise. It is the box's **squash-and-stretch open/bounce animation (`ive_box_scale` = `FUN_0046c86f`) caught at a different frame on each side at the capture anchor.** Decoded from `--d3d-trace-verts` (`intro-dialogue-lines --target both`, cap_00 = port f672 / retail f10849): box **texture, UVs, diffuse, and center are bit-identical**, but the **dst scale differs** — cap_00 port `sx 0.9875 / sy 1.0125` (= `ive_box_scale(box_open=15)` open branch, *exact*) vs retail `sx 1.0000 / sy 1.0000`; cap_01 port `sx 0.92 / sy 1.08` (narrow+tall, box_open≈4) vs retail `sx 1.06 / sy 0.94` (wide+short) — i.e. **~opposite bounce phases** (user-confirmed "way more squished on port", cap_01). Because the box is magnified 1.6× (1024/640), a slightly-differently-scaled box offsets the whole rim by a few px → the "halo" tracing the bubble outline. The geometry formula is faithfully ported; the divergent input is the **animation state** (`box_open`/the open-vs-collapse branch) at TEXT_ANIM_END — same family as opening-prologue note #6 "bubble bounce-in phase drift" + the db054 phase class (engine-quirks §94). **NEXT:** Frida `--watch` `box_open`=`DAT_073a3e14` + reveal cursor `DAT_073a6a38` per-frame on retail, align to anchor vs port, find which timing input diverges (inter-line box-collapse duration / reveal speed / per-frame sub-step count). | user-flagged 2026-06-02 ("halo … likely scaling filter") + 2026-06-05 confirmed squish at cap_01. Evidence: decoded box quad verts (above) + feed `box_overlay_cap01` (retail=green/port=red rects). The old "TGA-decode/MODULATE/POINT-vs-LINEAR ±1" hypothesis is **disproven** — verts show a real (sub-px→several-px) scale offset, not a texel-precision delta. |
| **Skip-prompt `savewindow.tga` banner ~−1 LSB (STILL OPEN, separate from box-scale row)** | A uniform ~−1 LSB on the gold (port `[237,200,52]` vs retail `[238,201,52]`) + text edges on the skip-prompt banner. **NOT** investigated under the 2026-06-05 box-scale root-cause (that was the dialogue `ive_window` bubble, a geometry/phase issue). This banner case is a uniform flat-colour offset → still suspect 2D-UI texture/colour precision (TGA decode/upload or MODULATE rounding). Chase: dump+diff the decoded `savewindow.tga` texels port-vs-retail; AND verify the banner box isn't *also* a scale/phase difference (decode its verts first before assuming colour). Invisible (±1). | user-flagged 2026-06-05 ("faint diff of the box, amplified by scaling"). Real delta — do NOT hand-wave. cf engine-quirks §54. |
