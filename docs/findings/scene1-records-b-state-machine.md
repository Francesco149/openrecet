...
| C8jb.5a | Phase B damage-roll prologue: velocity-derived KB factor (`0.7/sqrt(VX²+VZ²)`) + hit-history ring bump (per in-range collision) + slot TYPE==0x53 heavy-attack short-circuit (per-type +0x20 gate + npc_type != 0x22 + FUN_004319d6 cooldown → kill_age write to npc+0xb18 + DAT_0438bed8=4). | L35276-L35307 | ~120 | LANDED — kb_strength/damage_out/heavy_atk_count observable |
| C8jb.5b | Phase B general damage formula: base damage (DAT_056db0b4 idle / per-NPC-type +0x3c curve) × per-NPC-type +0x34 KB modifier × FUN_0041f46d RNG scale + 4× FUN_0043647f combo modifiers + scene-state mul DAT_056db014/01c + slot[7..0xc] knockback-state mods + LAB_00438e6d *2 chain. | L35308-L35385 | ~150 | LANDED — damage int computed for slot.TYPE != 0x53 |
| C8jb.5c | Phase B post-damage clamps: NPC 0x12 + 0x44/0x45 + sub-iter==0 + phase!=6 reset + charge bit (npc.charge_flag atan2-into-facing) + npc_phase 1-6 → local_1c bit 3 + atan2 quadrant bits (bit 0/1/2) + slot[1]==0 OWNER_A flag mods + LAB_004390d3 (local_18-armed) clamp + type-5 < 0 clamp + general < 1/<5 RNG clamp. | L35372-L35442 | ~160 | LANDED — full damage clamp/quadrant flags |
