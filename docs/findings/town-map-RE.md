...
|---|---|---|---|---|
| 1 | ~~**load fade-in** phase mismatch~~ **ACCEPT-AS-PHASE 2026-06-08** | 0→12 (resolves f16) | **PHASE (§85)** | On the re-windowed (entry-aligned) capture the load fade is a **sub-8/ch whole-frame tail over f0–12** that **resolves to bit-identical by f16** (full-frame diff 0 px >8/ch the whole way, mean→0.00 at f16). Exactly the §85 load-origin offset (`FUN_004526f5`/`DAT_0438bf78` starts a few frames off port↔retail), NOT a logic bug. Accept. |
| 2 | ~~**marker highlight** pulse phase mismatch~~ **RESOLVED → 1:1 2026-06-08** | — | **PHASE→aligned** | On the entry-aligned + `{phasepin}` window the markers are **bit-identical** (full-frame mean 0.00 f16→f638, markers included). The OLD divergence was the unaligned-frame artifact (entry timer `_DAT_09643628` resets to 0 at mode-8 entry on both sides → pulse phase matches once frames are paired at entry). |
| 3 | ~~**hand cursor** bob phase mismatch~~ **RESOLVED → 1:1 2026-06-08** | — | **PHASE→aligned** | Same: bit-identical in the aligned+pinned window (the trace's `{phasepin:0}` zeroes `DAT_0438b154` on both sides; full-frame diff 0). |
...
**ENTIRE world-map backlog now CLOSED (2026-06-08).** On the entry-aligned + `{phasepin}`
capture the full frame is **bit-identical f16→f638 (mean 0.00, 0 px >8/ch)** with only a
sub-8/ch load-fade tail over f0–12 (#1, §85, resolves f16). So #2 (marker pulse) + #3 (cursor
bob) are **1:1** (the old divergences were the unaligned-frame + unpinned artifacts the
multi-pillar method predicts — "normalize phase, THEN compare"), #4 nav was FIXED, #5 tooltip
DONE. The world map renders 1:1 vs retail across the whole captured nav. **DONE 2026-06-08 — re-windowed +
